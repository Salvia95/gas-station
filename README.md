...
> Java 17+도 Record, Pattern Matching 등으로 불필요한 보일러 플레이트를 줄여 생산성이 많이 개선되었지만, **Null Safety에 대해서는 여전히 컴파일 타임에 미리 방지 할 수 없다는 점**과 비동기 처리 등에서 시 코틀린(Coroutines 등)이 Java(CompletableFuture, Flow 등)에 비해 훨씬 직관적여서 **오용 가능성이 더 낮을 것이라 판단**했습니다.
...
> `MySQL`도 널리 사용되고 있는 오픈소스 데이터베이스지만, 본 프로젝트에서는 `Oracle`에서의 마이그레이션 관점에서 `Oracle`과 문법적 유사도가 높은 `PostgreSQL`이 쿼리 로직의 변환과 학습 곡선 면에서 더 적합하다고 판단했습니다.
